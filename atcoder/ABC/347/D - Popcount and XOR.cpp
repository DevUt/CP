...
void solve(int test_case) {
    int a,b;
    unsigned long long c; cin>>a>>b>>c;
    int m = popcntll(c);
    int l = a-b;
    if(((m-l)&1 )|| ((m+l)&1) || a >  b + m  || b > m + a || m > a + b){
        cout<<"-1";
        return;
    }
    int x = (m+l)/2;
    ...
    a -= x;
    b -= y;
    string newa(60,'0'),newb(60,'0');
    string strc = bitset<60>(c).to_string();
    for(int i=59;i>=0; i--){
        if(strc[i] == '1'){
            if(x ){
                newa[i] = '1';
    ...
    //cerr<<count(all(newa),'1')<<" "<<count(all(newb),'1')<<'\n';
    //cerr<<newa<<'\n'<<newb;
    cout<<stoll(newa,nullptr,2)<<' '<<stoll(newb,nullptr,2);
