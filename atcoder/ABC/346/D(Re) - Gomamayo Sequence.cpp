...
void solve(int test_case) {
    ...
    vector<ll> first_pref(n+1),second_pref(n+1), first_suf(n+1), second_suf(n+1);

    for(int i=0;i<n; i++){
        if(i&1){
            if(s[i] == '0'){
                first_pref[i+1] = first_pref[i] + v[i];
                second_pref[i+1] = second_pref[i];
            }
            else{
                second_pref[i+1] = second_pref[i] + v[i];
                first_pref[i+1] = first_pref[i];
            }
        }else{
            if(s[i] == '1'){
                first_pref[i+1] = first_pref[i] + v[i];
    ...
    for(int i=n-1;i>=0;i--){
        if(i&1){
            if(s[i] == '0'){
                first_suf[i] = first_suf[i+1] + v[i];
                second_suf[i] = second_suf[i+1];
            }else{
                second_suf[i] = second_suf[i+1] + v[i];
                first_suf[i] = first_suf[i+1];
            }
        }else{
            if(s[i] == '1'){
                first_suf[i] = first_suf[i+1] + v[i];
    ...
    for(int i=0;i<n-1; i++){
        ckmin(ans,min(first_pref[i+1]+second_suf[i+1],second_pref[i+1]+first_suf[i+1]));
    }
    cout<<ans;
  // cout<<"Case #"<<test_case<<": ";
}
