...
void solve(int test_case) {
  ...
  auto win = [](const char inp, const int id) {
    switch (id) {
    case 0: { // We have hoof
        if(inp == 'S')
          return 1;
    ...
    case 1: { // We have paper 
        if(inp == 'H')
          return 1;
        else
          return 0;
    }
    case 2: { // We have scissor
        if(inp == 'P')
          return 1;
        else
          return 0;
  ...
  for (int i = n - 1; i >= 0; i--) {
    for (int j = 0; j <= k; j++) {
      for (int l = 0; l < 3; l++) {
        // Don't change
        dp[i][j][l] = win(v[i], l) + dp[i + 1][j][l];
        // Change
        for (int m = 0; m < 3; m++) {
          if (m == l)
            continue;
          if (j > 0) // Can't change if at most 0 changes
            ckmax(dp[i][j][l], win(v[i], m) + dp[i + 1][j - 1][m]);
        }
      }
    }
  }
  cout << (*max_element(all(dp[0][k]))) << '\n';
  // cout<<"Case #"<<test_case<<": ";
}
