...
map<int,pair<int,int>> precedence;
int minSteps(int l, int r, int limit){
  if(l == r) return 0;
  ...
  while(!q.empty()){
    ...
    if(steps + 1 == limit)
      continue;
    array<int,3> nextnum = {current*2, current/2, current*2 + 1};
    for(int i : nextnum){
      if(i == r) return steps+1;
...
void solve(int test_case) {
  ...
  for(int i=0;i<n; i++){
    ...
    if(x == -2){
      ...
      int steps = minSteps(v[x],v[i],(i-x+1));
    }
  }
  // cout<<"Case #"<<test_case<<": ";
}
