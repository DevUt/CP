...
void solve(int test_case) {
  int n,m; cin>>n>>m;
  vector<vector<int>> g(n,vector<int>(m)); cin>>g;
  auto largest_neighbour = [&](int i, int j){
    vector<int> neighbours;
    if(i+1<n) neighbours.push_back(g[i+1][j]);
    if(i-1>= 0) neighbours.push_back(g[i-1][j]);
    if(j+1<m) neighbours.push_back(g[i][j+1]);
    if(j-1>=0) neighbours.push_back(g[i][j-1]);
    sort(all(neighbours));
    return neighbours.back();
  };
  for(int i=0;i<n;i++){
    for(int j=0; j<m; j++){
      ckmin(g[i][j], largest_neighbour(i, j));
    }
  }
  for(auto& row : g)
    cout<<row<<'\n';
