...
void solve(int test_case) {
  ...
  for(int i=sx.size()-1; i>0; i--){
    char tosearch;
    if(sx[i] == '0' && carryforward){
      cout<<"NO\n";
      return;
    }
    else
     tosearch = char(sx[i] - carryforward);
    if(table.contains(tosearch) == false){
      cout<<"NO\n";
      return;
    }
    ansa.push_back(table[tosearch].first);
    ansb.push_back(table[tosearch].second);
    carryforward = 1;
  }
  cout<<"YES\n";
  // cout<<"Case #"<<test_case<<": ";
