...
map<pair<ll, int>, ll> cach;
vector<int> v;
ll ansfn(ll c, int idx) {
  if (idx == v.size() - 1) {
    cach[{c, idx}] = max(c + v[idx], abs(c + v[idx]));
    return cach[{c, idx}];
  }

  if (cach.find({c, idx}) == cach.end())
    cach[{c, idx}] =
        max(ansfn(c + v[idx], idx + 1), ansfn(abs(c + v[idx]), idx + 1));
