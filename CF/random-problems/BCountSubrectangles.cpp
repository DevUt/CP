...
void solve(int test_case) {
    ...
    unordered_map<int, int> sega, segb;

    auto seglength = [](vector<int>& a, unordered_map<int, int>& b){
        int cnt = 0;
    ...
    for(auto& [x, cnta] : sega){
        for(auto& [y, cntb] : segb){
            for(int i=1; i<= x; i++){
                if(k%i != 0)
                    continue;
                ll width = max((y - k/i + 1),0);
                ll height = max((x-i+1),0);

                ans += (cnta*width*cntb*height);
            }
