...
array<array<ll, 2>, 1'000'001> dp;

#define int ll
constexpr void build() {
  dp[1][0] = 1;
  dp[1][1] = 1;
  auto checkIfTriangular = [](int x) -> bool{
    int m = 8*x + 1;
    return (ll) sqrt(m) * (ll)sqrt(m) == m;
  };
  for (int i = 1; i < 1414; i++) {
    int lastT = i * (i + 1) / 2;
    for (int j = lastT + 1; j <= lastT + i+1 && j < dp.size(); j++) {
      if(checkIfTriangular(j)){
        dp[j][0] = j*j;
        dp[j][1] = dp[j][0] + dp[lastT][1];
      }else if(checkIfTriangular(j-1)){
        dp[j][0] = dp[j-i][0] + j*j;
        dp[j][1] = dp[j][0];
      }else{
        dp[j][0] = dp[j-i][0] + j*j;
        dp[j][1] = dp[j-i-1][1] + dp[j][0];
      }
    }
  }
}
