...
void solve(int test_case) {
  ...
  multiset<ll> supports;

  vector<ll> dp(m);
  vector<ll> costofeach(n);
  for(int row=0; row<n;row++){
    supports.clear();
    supports.insert(1); // Value at v[row][0];
    dp[0] = 1;
    for(int i=1; i<m; i++){
      if(supports.size() > d+1){
        auto it = supports.find(dp[i-d-2]);
        supports.erase(it);
      }
      auto small = supports.begin();
      dp[i] = v[row][i] + 1 + (*small); 
      supports.insert(dp[i]);
    }
    costofeach[row] = dp[m-1];
    fill(all(dp),0);
  }
