...
void solve(int test_case) {
    int n,k; cin>>n>>k;
    string s,t; cin>>s>>t;
    vector<vector<int>> gs(n,vector<int>());
    vector<char> gsmark(n);
    for(int i=0; i+k<n;i++){
        gs[i].push_back(i+k);
        gs[i+k].push_back(i);
        if(i+k+1<n){
            gs[i].push_back(i+k+1);
            gs[i+k+1].push_back(i);
        }
    }
    ...
    for(int i=0; i<n; i++){
        if(gsmark[i])
            continue;
        dfsStack.push(i);
        multiset<char> sset, tset;
        while(!dfsStack.empty()){
            if(gsmark[dfsStack.top()]){
                dfsStack.pop();
                continue;
            }
