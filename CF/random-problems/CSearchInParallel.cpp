...
void solve(int test_case) {
    ...
    for(int i=0; i<n; i++){
        sumone = s1*(lists[0].size()+1)*v[i].first;
        sumtwo = s2*(lists[1].size()+1)*v[i].first;
        if(sumone < sumtwo){
            lists[0].push_back(v[i].second);
        }else{
            lists[1].push_back(v[i].second);
        }
    }

    cout<<lists[0].size()<<' '<<lists[0]<<'\n';
    cout<<lists[1].size()<<' '<<lists[1]<<'\n';
  // cout<<"Case #"<<test_case<<": ";
}
