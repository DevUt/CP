...
void solve(int test_case) {
    ...
    auto comp = [](const pair<int,int>& afirst, const pair<int,int>& b) {
        if(afirst.first == b.first)
            return afirst.second > b.second;
        else
         return afirst.first > b.first;
    };
    priority_queue<pair<int,int>,vector<pair<int,int>>,decltype(comp)> sc;
    for(int i=0; i<n; i++){
        sc.push({c[i],i});
    }
    ...
    while(m--){
        ...
        cost += sub*c[j];
        while(d > 0 && sc.size()){
            while(sc.size() && !a[sc.top().second]){
                sc.pop();
            }
            if(sc.empty())
                break;
            sub = min(d,a[sc.top().second]);
            d -= sub;
            a[sc.top().second] -= sub;
            cost += sc.top().first*sub;
            if(a[sc.top().second] == 0)
                sc.pop();
        }
        if(d > 0){
            cost = 0;
