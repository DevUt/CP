...
void solve(int test_case) {
    ...
    while(true){
        if(turn){
            ...
            int idx = -1;
            for(int i=0; i<n; i++){
                if(bob[i] >= elem && alice[i] != 0){
                    if(bob[i] == elem && alice[idx] > alice[i]){
                        continue;
                    }
                    idx = i;
                    elem = bob[i];
            ...
            int idx = -1;
            for(int i=0; i<n; i++){
                if(alice[i] >= elem && bob[i] != 0){
                    if(alice[i] == elem && bob[idx] > bob[i])
                        continue;
                    idx = i;
                    elem = alice[i];
                }
    ...
    ll a = accumulate(all(alice),0ll);
    ll b = accumulate(all(bob), 0ll);
    cout<<a-b<<'\n';
