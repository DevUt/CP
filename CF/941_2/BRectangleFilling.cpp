...
void transpose_inplace(std::vector<std::vector<char>>& data) {
  // Determine the number of columns and rows (assuming all inner vectors have the same size)
  int num_cols = data.empty() ? 0 : data[0].size();
  int num_rows = data.size();

  // Check if dimensions require transpose (avoid unnecessary work)
  if (num_cols <= num_rows) {
    return;
  }

  // Create a temporary vector to hold transposed data (more efficient)
  std::vector<std::vector<char>> transposed(num_cols, std::vector<char>(num_rows));

  // Iterate and populate the transpose
  ...
  // Swap the original data with the transposed data (optional)
...
void solve(int test_case) {
  ...
  auto is_allsame = [](vector<char> &x) {
    return (count(all(x), 'W') == x.size()) || (count(all(x), 'B') == x.size());
  };

  int badcnt = 0;
  int i = 2;
  while (i--) {
    if (is_allsame(v[0]) && is_allsame(v[v.size() - 1])) {
      if (v[0] != v[v.size() - 1])
        badcnt++;
    }
    transpose_inplace(v);
