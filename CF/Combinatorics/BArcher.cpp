...
public:
    ...
    auto operator<=>(const frac& f) const{
        return (a*f.b <=> b*f.a);
    }
...
void solve(int test_case) {
    array<int,4> a; cin>>a;
    frac smallR = frac(a[0],a[1]);
    frac zcom = frac(1,1) - frac(a[2],a[3]);
    frac smallRcom = frac(1,1) - smallR;

    frac ans = smallR *(frac(1,1)/(frac(1,1)-(zcom*smallRcom)));
    cout<<setprecision(10)<<ans.toLDouble();
