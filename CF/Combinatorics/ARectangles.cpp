...
void solve(int test_case) {
    ...
    ll ans = 0;
    for(auto& x : v){
        ll numone = count(all(x), 1);
        ll numzer = count(all(x), 0);

        ans += (1LL<<numone)-1 + (1LL<<numzer)-1;
    }
    for (int i=0; i<m; i++){
        ll numone = 0, numzer =0;
        for(int j=0; j<n; j++){
            if(v[j][i] == 1)
                numone++;
            else
        ...
        ans += max((1LL<<numone)-(1LL+numone),0ll) + max((1LL<<numzer)-(numzer+1),0ll);
    }
    cout<<ans<<'\n';
