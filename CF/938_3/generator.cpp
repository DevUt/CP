...
void solve(int test_case) {
    ...
    array<int,2> nm = {0,0};
    fill_with_random_int_values(all(nm),1,20);
    if(nm[0] < nm[1])
        swap(nm[0],nm[1]);
    ...
    vector<int> a(n), b(m);
    fill_with_random_int_values(all(nm),1,100);
    if(nm[0] < nm[1])
        swap(nm[0],nm[1]);

    fill_with_random_int_values(all(a), nm[1], nm[0]);
    fill_with_random_int_values(all(nm),1,100);
    if(nm[0] < nm[1])
        swap(nm[0],nm[1]);
    fill_with_random_int_values(all(b), nm[1], nm[0]);
    cout<<n<<' '<<m<<' '<<k<<'\n';
    cout<<a<<'\n'<<b<<'\n';
  // cout<<"Case #"<<test_case<<": ";
}
