...
void solve(int test_case) {
    ...
    int v_instance = 200;
    int a_instance = -1;
    vector<int> i_instance;
    vector<int> k_instance;

    // find earliest instance of 'v'
    for(auto &row : a){
        for(int i=0; auto &val : row){
            switch (val) {
                case 'v':
                    ckmin(v_instance, i);
                    break;
                case 'i':
                    i_instance.push_back(i);
                    break;
                case 'k':
                    k_instance.push_back(i);
                    break;
                case 'a':
                    ckmax(a_instance, i);
                default:
    ...
    for(auto &i : i_instance){
        bool found = false;
        for (auto &k : k_instance) {
            if(i > v_instance && i < k && k < a_instance){
                found = true;
            }
        }
        if (found) {
