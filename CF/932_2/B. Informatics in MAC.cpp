...
void solve(int test_case) {
    ...
    set<int> firstMex, secondMex;
    for(int i=0;i<=n;i++){
        firstMex.emplace_hint(firstMex.end(), i);
        secondMex.emplace_hint(secondMex.end(), i);
    }
    ...
    firstMex.erase(v[l]);
    while(l != n){
        if((*firstMex.begin()) == (*secondMex.begin())){
            cout<<2<<'\n'<<"1 "<<l+1<<'\n'<<r+1<<' '<<n<<'\n';
            return;
        }
        l++;
